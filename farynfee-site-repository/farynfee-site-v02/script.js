...
// Movement stills use the same fullscreen viewer.
const stillButtons = [...document.querySelectorAll(".still-open")];

stillButtons.forEach((button) => {
  button.addEventListener("click", () => {
    const allButtons = [...document.querySelectorAll(".diary-open, .still-open")];
    const index = allButtons.indexOf(button);
    diaryButtons.splice(0, diaryButtons.length, ...allButtons);
    openDiary(index);
  });
});
